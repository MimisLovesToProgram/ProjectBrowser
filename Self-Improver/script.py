...
if action == "1":
    ...
    if not f"------------------------------------------------------------->{title}\n" in prevlines:
        with open(f"{os.getcwd()}\\Self-Improver\\savefile.txt", "a") as f:
    ...
    # If there is a goal with this title, reset it with the data provided by the user.
        ...
        with open(f"{os.getcwd()}\\Self-Improver\\savefile.txt", "w") as f:
            f.writelines(lines)
    ...
    with open(f"{os.getcwd()}\\Self-Improver\\savefile.txt", "w") as f:
        # Declaring modification variables, and checking whether we are iterating over the correct lines of 'savefile.txt', just like above.
        minutes = 0
        goal = 0
        start_date = ""
        addition = ""
